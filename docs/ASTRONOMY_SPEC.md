...
|---|---|---|
| Catalogue/version | The author-replacement files dated 2008-09-16 for CDS/VizieR I/311, *Hipparcos, the New Reduction*, remain the Phase 1 local-spike source only. Milestone 2D.1 identifies ESA Gaia DR3 version 1.1 `gaiadr3.gaia_source` as the preferred single-source-first production candidate, not an active authority. A bounded official TAP screen preliminarily found 8/19 technical HIP matches and five finite-RV candidates, but no Polaris match and no approved row; without retained query text, response/canonical extraction, acquisition timestamp, hashes and manifest, those counts are not reproducible 2D authority. Approval remains blocked on immutable acquisition, physical-component/quality/systematics review, the scale-aware TCB-to-TDB mapping, an exact Polaris fallback, and exact derived-artifact rights interpretation. Raw/query-derived material remains local and ignored until approved. | `CATALOGUE_AUTHORITY_BLOCKED`; Gaia DR3 preferred candidate, source rows and deployment rights still 2D |
...
| Catalogue reference epoch | Every source-neutral row carries the selected release's declared epoch representation and time scale. The preferred Gaia DR3 candidate declares a TCB-parametrized Julian `J2016.0` reference epoch and proper-motion state. IAU B3 supplies the TCB-to-TDB time relation, but an epoch-only conversion is insufficient: a pinned scale-aware parameter/uncertainty/covariance mapping into the approved TDB propagation interface remains a 2D.1 gate. For the I/311 spike, carry literal `Ep=1991.25` separately: ESA Gaia DR1 establishes only its Julian `J1991.25` representation, not an I/311-applicable time scale. Never transfer the original catalogue's `J1991.25(TT)` or let a library default decide a missing scale. | Gaia source semantics and IAU time relation `SOURCE_SUPPORTED_FACT`; exact adapter and row approval `BLOCKS_2D_DATA_AUTHORITY`; I/311 time scale `AUTHORITY_OR_EVIDENCE_MISSING` / `HUMAN_REVIEW_REQUIRED` |
| Space motion | Source `mu_alpha_star = (d alpha / dt) cos(delta)`, Dec motion, derivative time coordinate/unit, positive approved parallax/equivalent distance, and finite approved RV must all be explicit. Gaia DR3 defines TCB-parametrized `pmra` as the local tangent-plane starred-alpha component and `pmdec` toward increasing declination; row-specific parallax systematics, applicable astrometric covariance, separate RV uncertainty, RV sign/systemic suitability, and the TCB/TDB adapter still require review. Missing astrometry-RV cross-covariance is `UNKNOWN_NOT_PROVIDED`, never zero; it needs a reviewed omission but is not a fictional mandatory full covariance. I/311 Appendix G Table G.3 independently defines its `pmRA` as starred alpha in mas/yr but does not close its time-scale or RV gaps. The selected SOFA-derived boundary converts to coordinate rate only away from the exact pole, preserves every status, and fails closed on missing/unapproved state. | Generic adapter/full-input route `PROJECT_DECISION`; Gaia/I/311 adapter and row authority `BLOCKS_2D_DATA_AUTHORITY`; numerical acceptance postimplementation |
...
- `CatalogueIcrsState{raDeg,decDeg,sourceEpochLabel,epochRepresentation,
  epochScale,properMotionRaCosDec,properMotionDec,derivativeScale,
  approvedPositiveParallax,approvedRadialVelocity,uncertainties,covariance,
  sourceAndApprovalProvenance,...}`;
