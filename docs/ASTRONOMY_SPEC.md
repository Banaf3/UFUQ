...
|---|---|---|
| Catalogue/version | Use CDS/VizieR I/311, *Hipparcos, the New Reduction*, as the sole catalogue source for the Phase 1 local technical spike. This does not approve source-derived redistribution, deployment, parser fields, filtering, a runtime subset, or production use. | APPROVED FOR PHASE 1 LOCAL TECHNICAL SPIKE; remaining AST-001 policy unresolved |
| Catalogue identifier | Preserve source-defined stable identifiers from I/311; cultural names are aliases, never coordinate keys. Exact retained fields remain an AST-001 decision. | CLARIFIED; field scope unresolved |
...
| Catalogue reference epoch | Carry I/311's `Ep=1991.25` semantics separately from frame/equinox and observation time. Confirm the exact time-scale interpretation during the spike; do not transfer the original catalogue's fuller epoch statement or collapse “Julian epoch” and “time scale” into one label. | SOURCE-DEFINED LABEL; AST-003 experiment required |
| Space motion | For proper motion, parallax, radial velocity, and perspective acceleration, record an explicit implement-or-omit decision and quantified bound over the supported date range. Preserve component conventions and quality/uncertainty flags. I/311 `pmRA` has `STRONG_SUPPORT_BUT_SPIKE_CONFIRMATION_REQUIRED`; test omitted and double application of `cos(delta)` rather than choosing from memory. | MANUAL DOMAIN DECISION AST-003; experiment required |
