...
| UFUQ-SCIENTIFIC-PROFILE-V1 | `docs/spikes/PHASE1_SCIENTIFIC_PROFILE_V1.md`, Milestone 2C.6 profile and exit-gate audit | PROJECT_DECISION | REQUIRED_NOW | Defines the minimal one-preset, bounded-time, source-neutral, geometric-only profile; normative boundary/exclusion/outcome/pending-validation semantics; complete A-F blocker triage; three-stage validation lifecycle; 49-term roadmap disposition; resource actions; and exact 2C exit criteria. Its route, UTC/date domain, and leap/EOP policy remain review-gated; it is not a data source, production output, scientific acceptance, or numerical tolerance. |
