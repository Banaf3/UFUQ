...
| Gaia DR3 TCB-to-TDB compatible quantities | Gaia DR3 TCB/reference-system and motion-model documentation; IAU 2006 B3; IAU 2012 B2; Klioner (2008/2010); IAU 2000 C1; Lindegren/Dravins (2003); SOFA `2023-10-11` preambles | `TCB_TDB_ADAPTER_AUTHORITY_CLOSED`: preserve direction; convert the same epoch event; map proper-motion rates and catalogue parallax by `1/(1-L_B)`, coordinate distance by `1-L_B`, and covariance by the explicit Jacobian; preserve spectroscopic RV without compatible scaling or automatic kinematic approval. Row/RV-proxy authority remains 2D gate C. |
