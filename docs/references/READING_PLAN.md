...
| `ESA-GAIA-DR3-CANDIDATE` | ESA Gaia DR3 dataset version 1.1/DOI record, documentation release 1.3, astrometric TCB/reference-system section, `gaia_source`, Hipparcos-2 crossmatch, credit/licence, archive, and DR4-status pages | Searchable official web | `CRITICAL_NOW`: fixed release identity; TCB/ICRS/epoch/PM/parallax/systematics/five-/six-parameter-covariance/RV/quality semantics; bright/RV coverage; release-scoped IDs/crossmatch; access, attribution and redistribution/deployment terms | Studied release evidence and a bounded official TAP screen. Gaia DR3 remains preferred; the preliminary screen found 8/19 matches, five finite-RV candidates and no Polaris match, but retained no exact query/result manifest and approved no row. Scale-aware TCB-to-TDB mapping, immutable acquisition, row/fallback review, and exact derived-artifact rights interpretation remain required. |
| `IAU-TDB-2006-B3` and `KLIONER-SCALING-2008` | Official IAU TDB definition and peer-reviewed relativistic compatible-quantity treatment | Searchable official/primary web | `CRITICAL_NOW`: distinguish TCB-to-TDB time conversion from the associated parameter/rate/covariance mapping | Studied. Epoch-only relabelling is prohibited; exact Gaia stellar-parameter/covariance mapping into the SOFA TDB interface remains astronomy-review gated. |
