...
| DATA-SRC-001 | Approve the production catalogue/release, bounded row set, source-pinned TCB-to-TDB and RV mappings, physical-component matches, row solution/quality/uncertainty/covariance, local/deployment rights, and stable UFUQ `starId`/per-field-source crosswalk. | `CATALOGUE_AUTHORITY_BLOCKED`: Milestone 2D.1 prefers ESA Gaia DR3 version 1.1 `gaiadr3.gaia_source`. Official rights evidence says open/free with credit while the DOI record separately declares CC BY-NC 3.0 IGO; local analysis/attribution are established, but derived deployment/public tracking require exact rights interpretation, noncommercial FYP use is only a candidate pending licence application, and commercial reuse is not approved. A bounded TAP screen preliminarily found 8/19 matches, all with positive raw parallax, five with finite RV/error, and no `HIP 11767` match, but retained no exact query/result manifest and is not reproducible 2D authority. No row is approved. Missing astrometry-RV cross-covariance is explicit `UNKNOWN_NOT_PROVIDED`, never zero and not automatically row-blocking absent a later error-budget requirement. I/311 remains the local spike. | Close exactly five gates: A. approve the TCB-to-TDB compatible-quantity adapter; B. retain immutable Gaia query/response authority; C. approve row-by-row eligibility and a minimal subset; D. approve direct ESA Hipparcos 1997 astrometry plus one separate primary systemic-RV authority for Polaris, or make that route ineligible; E. approve the exact derived-artifact rights interpretation. Do not create a blanket mixed catalogue. The complete stop rule is `spikes/PHASE1_PRODUCTION_CATALOGUE_AUTHORITY.md`. |
