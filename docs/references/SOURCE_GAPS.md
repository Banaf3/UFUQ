...
| AST-SRC-002 | Official Astropy coordinate and IERS documentation | `CLOSED_FOR_2C.1_REFERENCE_DESIGN`: exact official pages for Astropy `8.0.1` time formats/scales, space motion, and IERS status/warnings are recorded with retrieval version/date. Production route selection remains a separate AST-003 decision. | `studies/astropy-pyerfa-reference-docs.md` and `ASTROPY-DOCS-PIN` |
