...
| DATA-SRC-001 | Approve the production catalogue/release, bounded row set, source-pinned TCB-to-TDB and RV mappings, physical-component matches, row solution/quality/uncertainty/covariance, local/deployment rights, and stable UFUQ `starId`/per-field-source crosswalk. | `CATALOGUE_AUTHORITY_BLOCKED`: Milestone 2D.1 prefers ESA Gaia DR3 version 1.1 `gaiadr3.gaia_source`. Milestone 2D.1A closes the analytic compatible-quantity adapter without approving a row: it converts the same epoch event, maps proper motions/parallax/distance/covariance, and preserves the spectroscopic-RV observation without automatic kinematic approval. Official rights evidence says open/free with credit while the DOI record separately declares CC BY-NC 3.0 IGO; local analysis/attribution are established, but derived deployment/public tracking require exact rights interpretation. A bounded TAP screen preliminarily found 8/19 matches, all with positive raw parallax, five with finite RV/error, and no `HIP 11767` match, but retained no exact query/result manifest and is not reproducible 2D authority. No row is approved. Missing astrometry-RV cross-covariance is explicit `UNKNOWN_NOT_PROVIDED`, never zero. I/311 remains the local spike. | Adapter gate A is closed. Close four remaining gates: B. retain immutable Gaia query/response authority; C. approve row-by-row eligibility, component-specific RV proxies, and a minimal subset; D. approve direct ESA Hipparcos 1997 astrometry plus one separate primary systemic-RV authority for Polaris, or make that route ineligible; E. approve the exact derived-artifact rights interpretation. Do not create a blanket mixed catalogue. The complete stop rule is `spikes/PHASE1_PRODUCTION_CATALOGUE_AUTHORITY.md`. |
