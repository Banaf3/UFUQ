...
| AST-SRC-002 | Official Astropy coordinate and IERS documentation | `CLOSED_FOR_2C.1_AND_2C.2_REFERENCE_DESIGN`: exact official pages for Astropy `8.0.1` time formats/scales, space motion, GCRS/CIRS/ITRS/AltAz transformation roles, and IERS status/warnings are recorded with retrieval version/date. Astropy remains reference-only. | `studies/astropy-pyerfa-reference-docs.md` and `ASTROPY-DOCS-PIN` |
