...
| AST-SRC-007 | Leap-second/time-scale policy | `TIME_CONTRACT_RESOLVED_EOP_POLICY_OPEN`: source-defined UTC/TAI/TT/UT1 roles, RFC 3339's broader syntax, IERS Bulletin C authority, SOFA quasi-JD/conversion behavior, and Astropy reference behavior are recorded. ScientificProfileV1 normatively selects whole-second `YYYY-MM-DDTHH:mm:ssZ`, conditional exact-date validation of `23:59:60Z`, typed UTC quasi-JD/TAI/TT/UT1 states, no implicit local/current time or unlabeled JD, explicit conversion evidence, and fail-closed outcomes. It selects no production artifact family/bytes/hash, EOP interpolation/quality policy, expiry rule, Bulletin C cross-check, or update cadence. | **PRE_IMPLEMENTATION:** approve the remaining leap/EOP artifact-class, field, source-quality, interpolation, validity/expiry/update/offline, and warning policy. **2D:** select/acquire the machine-readable artifact versions/hashes and Bulletin C cross-check; 2D/2E derives the activated bounds. Wall-time UX is a later adapter concern. |
...
|---|---|
| `A BLOCKS_SCIENTIFIC_PROFILE_V1` | `AST-SRC-004`, `AST-SRC-007`, `AST-SRC-010` |
| `B BLOCKS_2D_DATA_AUTHORITY` | `AST-SRC-006`, `AST-SRC-009`, `AST-SRC-011`, `AST-SRC-012`, `DATA-SRC-001`, `DATA-SRC-002` |
