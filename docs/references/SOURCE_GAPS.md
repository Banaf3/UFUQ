...
| AST-SRC-010 | Production transformation route and implement-or-omit effect decision | `PARTIAL_PROPOSAL`: Milestone 2C.2 defines a componentized SOFA `2023-10-11` CIO-family semantic candidate, classifies every effect, and separates the Astropy/PyERFA reference route. Milestone 2C.5A classifies the eight route/effect experiment families without running them: same-family synthetic measurements cannot approve the production route or omission bounds. No production TypeScript implementation/library, reviewer approval, experiment result, omission bound, supported range, or tolerance is selected. | AST-003 approval of the final route/library/effect matrix and warning mapping; reviewed 2C.5A protocol and completed applicable experiments; AST-006 error budget/tolerances; implementation/reference comparison evidence |
...
| AST-SRC-014 | Scientific experiment execution and acceptance evidence | `PARTIAL_PROPOSAL`: Milestone 2C.5A freezes 24 human/machine experiment records plus fixture/result schemas, classifies 17 as synthetic-runnable, five as project-decision blocked, one as required-data blocked, and one as deferred to production, and proposes a five-group 2C.5B batch. It records Astropy/PyERFA shared ERFA/SOFA lineage and forbids numerical acceptance before AST-006. No experiment body/result, independent multi-model refraction path, production comparison, error-budget ledger, or tolerance exists. | Review the protocol and machine records; if approved, execute and review the proposed synthetic batch; pin missing independent models/data; resolve project decisions; add source-derived and production/reference cases only after their gates; approve a separate AST-006 error-budget ledger and thresholds |
