...
| AST-SRC-005 | Atmospheric-refraction assumptions | `DEFERRED_FROM_PROFILE_V1`: SOFA `iauRefco` input units/model form and `iauAtioq` numerical guard plus Astropy `8.0.1` pressure/default/low-altitude behavior are pinned. Milestone 2C.6 proposes geometric-only V1 with refraction disabled and no atmosphere defaults. Library defaults/guards and the documented about-5-degree region are not UFUQ policy, range, or tolerance. | Required only before a later refraction extension: AST-004/006 approval of model/version, measured/derived input provenance and uncertainty, inputs/height/lapse, environment/altitude domain, near/below-horizon behavior, warnings, bounds, and experiments |
...
| AST-SRC-010 | Production transformation route and implement-or-omit effect decision | `PARTIAL_PROPOSAL`: Milestone 2C.2 defines a componentized SOFA `2023-10-11` CIO-family semantic candidate, classifies every effect, and separates the Astropy/PyERFA reference route. Milestone 2C.6 narrows the preimplementation closure to approval of the `ScientificProfileV1` route/library mapping, effect dispositions, inputs, exclusions, and warnings. No TypeScript implementation is required to make that decision. | **PRE_IMPLEMENTATION:** AST-003 profile/route/effect approval. **POST_IMPLEMENTATION:** production/reference comparison, implementation residuals, error-budget population, and tolerance approval. Remaining later-effect experiments do not block V1 when their effects are excluded. |
...
| AST-SRC-012 | Production observer and endpoint domain | `PARTIAL_PROPOSAL`: Milestone 2C.3 defines an explicit geodetic observer boundary with east-positive longitude, datum/ellipsoid, ellipsoidal height, provenance, uncertainty, invalid-versus-outside-domain outcomes, and no silent orthometric conversion. No datum, longitude representative, height/location range, polar policy, uncertainty rule, or unrestricted/global claim is approved. | **PRE_IMPLEMENTATION:** approve one named preset record with datum/frame/ellipsoid, east-positive longitude representation, ellipsoidal height, uncertainty/provenance, and reject-all-other-locations behaviour. **LATER:** approve multiple/global ranges, poles, arbitrary locations, and their boundary fixtures. |
...
|---|---|
| `A BLOCKS_SCIENTIFIC_PROFILE_V1` | `AST-SRC-004`, `AST-SRC-006`, `AST-SRC-007`, `AST-SRC-010`, `AST-SRC-012` |
| `B BLOCKS_2D_DATA_AUTHORITY` | `AST-SRC-009`, `AST-SRC-011`, `DATA-SRC-001`, `DATA-SRC-002` |
